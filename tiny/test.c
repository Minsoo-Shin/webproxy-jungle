...
void doit(int fd)
{
  ...
  rio_t rio, server_rio; //server_rio가 따로 있다. 
...
// https://hostname:port/path 를 parsing하는 과정
void parse_uri(char *uri, char *hostname, char *path, int *port)
{
  *port = 80;
  char *pos = strstr(uri, "//"); 
...
// proxy가 이번엔 클라이언트 입장이 되어 서버에게 전송하기 위해 connect 시도
